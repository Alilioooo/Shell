...
echo "------------------------------"
echo "$(lscpu | grep 'CPU(s):' | head -n1 | column)"
echo ""
...
echo "------------------------------"
# 获取根文件系统的设备名称
root_device=$(lsblk -P | grep 'MOUNTPOINT="/"')
...
echo "------------------------------"
#仅显示SCSI设备大小
disk_Scsi=$(lsscsi -s | awk '$NF ~ /[0-9]/ {size[$NF]++} END {for (s in size) printf "机械 %s硬盘：%d块\n", s, size[s]}' | column  -t)
if lsscsi -s | grep -q 'disk'; then
  echo "$disk_Scsi"
else
  echo "机械 硬盘：0块"
fi
...
#仅显示Nvme硬盘
disk_Nvme=$(lsblk -o NAME,SIZE | grep -Ev "$new_device_name|NAME" | grep nvme | awk '{size[$2]++} END {for (s in size) printf "Nvme %s硬盘：%d块\n", s, size[s]}'|column -t)
if  lsblk -o NAME,SIZE | grep -qi nvme ; then
  echo "$disk_Nvme"
else
  echo "Nvme 硬盘：0块"
fi
